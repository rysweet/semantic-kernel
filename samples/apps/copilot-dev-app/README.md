...
## Overview
The Copilot Dev application simulates an entire software development team using large language model (LLM) AIs. The app includes various roles such as program management, lead architect, engineering manager or scrum master, programmers, code reviewers, testers, and devops or infrastructure personnel.

The interactions with the LLM AIs will evolve as the user and the application engage in conversation, providing a comprehensive experience simulating a development team.
...
3. The sample uses two applications, a front-end web UI, and a back-end API server.
    ...
    1. Navigate to `\samples\apps\copilot-dev-app\webapi`
...
4. Build the back-end API server by following these instructions:
    1. In the terminal navigate to  `\samples\apps\copilot-dev-app\webapi`
...
5. Now that the back-end API server is setup, and confirmed operating, let’s
   proceed with setting up the front-end WebApp.
    1. Navigate to `\apps\copilot-dev-app\webapp`
